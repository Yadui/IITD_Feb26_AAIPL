...
class QuestioningAgent(object):
    def __init__(self, use_lora=False, lora_path=None, **kwargs):
        self.agent = QAgent(
            use_lora=use_lora,
            lora_path=lora_path,
            **kwargs
    ...
    inc_samples: List[Dict[str, str]] | None = None,
        ...
        if wadvsys:
            sys_prompt = (
                "You are an expert-level examiner generating extremely difficult MCQs. "
                "You must strictly follow output format instructions. "
                "Do not reveal reasoning outside JSON."
            )
        else:
            sys_prompt = "You are an examiner generating difficult MCQs."
        ...
        prompt = f"""Generate a logically valid, deterministic MCQ on topic: {topic}.

                The question must be internally consistent and mathematically or logically well-defined.
                
                Exactly one option must be logically correct.
                All other options must be strictly false under classical interpretation.
                
                If the logic is ambiguous, non-deterministic, or allows multiple valid answers,
                you must silently correct it before producing output.
                
                For syllogisms:
                - Use 4–5 premises.
                - At least one existential premise ("Some").
                - Conclusion must require chaining across at least 3 premises.
                - Avoid trivial deductions.
                
                For series:
                - The rule must be expressible as a strict function.
                - The pattern must be deterministic and mathematically describable.
    ...
    def filter_questions(
        ...
        def normalize_schema(q):
            # Already correct schema
            if isinstance(q, dict) and "choices" in q and "question" in q:
                return q

            # Nested LoRA schema
            if isinstance(q, dict) and "question" in q and isinstance(q["question"], dict):
                try:
                    stem = q["question"]["stem"]
                    options = q["question"]["options"]
                    answer = q["question"]["correct_option_id"]

                    return {
                        "topic": q.get("topic", ""),
                        "question": stem,
                        "choices": [
                            f"{opt['option_id']}) {opt['content']}"
                            for opt in options
                        ],
                        "answer": answer,
                        "explanation": q.get("answer_explanation", {}).get(
                            "brief_reasoning", ""
                        ),
                    }
                except:
        ...
        def basic_checks(q2: Dict[str, str]) -> bool:
            ...
            if (
                check_len
                + self.count_tokens_q(q2.get("explanation", ""))
                > 1024
            ):
                return False
        ...
        for i, q in enumerate(questions):
            ...
            if isinstance(q, dict):
                q_norm = normalize_schema(q)
                if q_norm and basic_checks(q_norm):
                    correct_format_question.append(q_norm)

            elif isinstance(q, str):
                try:
                    q1 = json.loads(q)
                    q1 = normalize_schema(q1)
                    if q1 and basic_checks(q1):
                        correct_format_question.append(q1)
                except json.JSONDecodeError:
                    print(f"Skipping invalid JSON at index {i}")
                    continue
